// ============================================================================
// Zero-Friction CMS — App wiring reference. This shows (a) the FloatingEditButton
// with the "Apply to code" panel, and (b) how to nest the providers + overlay in
// your root App component. Copy FloatingEditButton into App.tsx (or its own file)
// and wrap your <Router /> as shown at the bottom.
//
// Requires lucide-react and the shadcn toast hook (@/hooks/use-toast).
// ============================================================================
...
export function FloatingEditButton() {
  ...
  const buildPrompt = () => {
    const lines = pending
      ...
      .join("\n");
    return `Apply the pending CMS edits below to the source code, then delete them from the site_content table so they don't double-apply.\n\nFor each edit: find the old text in the relevant page/component, replace it with the new text, and confirm what you changed. The key encodes the route it was edited on: 'auto::/::' is the route mounted at '/', 'auto::/some-path::' is that path.\n\nPending edits:\n${lines}`;
  };
      ...
      {showApply && (
        <div className="bg-white text-zinc-800 rounded-xl shadow-xl border border-zinc-200 w-96 max-h-[28rem] overflow-hidden flex flex-col">
          <div className="px-4 py-3 border-b border-zinc-100 flex items-center justify-between">
            ...
            <button onClick={() => setShowApply(false)} className="text-zinc-400 hover:text-zinc-700" data-testid="button-close-apply">
              <X size={16} />
            </button>
          </div>
          <div className="overflow-auto flex-1 px-4 py-2 text-xs space-y-2">
            {pending.length === 0 && <div className="text-zinc-400 py-6 text-center">No pending edits.</div>}
            {pending.map(([k, v]) => (
...
// --- Provider nesting in your root App component ---
// Order matters: EditModeProvider must wrap ContentProvider (content-context
// reads editToken from edit-mode-context). Mount AutoEditOverlay + the button
// once, as siblings of your <Router />.
//
// function App() {
//   return (
//     <QueryClientProvider client={queryClient}>
//       {/* ...your theme/tooltip providers... */}
