...
interface EditableLinkProps {
  ...
  className?: string;
  /**
   * Pass your router's Link (e.g. wouter `Link`, react-router `Link`, or
   * Next.js `Link`) for SPA navigation. Falls back to a plain `<a>` which
   * triggers a full page load.
   *
   * For interop, the component is invoked with `href`, `children`, `className`,
   * `target`, `rel`, and `onClick`. Wouter passes `href` natively; for
   * react-router wrap your Link as `(props) => <RRLink to={props.href} {...props} />`.
   */
...
export function EditableLink({
  ...
  // Decide whether to use the user's router Link or a plain anchor.
  // External / mailto / tel always use a plain anchor regardless of router.
